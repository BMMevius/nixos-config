{ pkgs, ... }:
{
  ...
  services.greetd = {
    ...
    settings = {
      initial_session = {
        command = "${pkgs.uwsm}/bin/uwsm start -D Hyprland hyprland-uwsm.desktop";
        user = "bastiaan";
      };
      default_session = {
        # This is the "fallback" if you log out
        command = "${pkgs.tuigreet}/bin/tuigreet --time --cmd '${pkgs.uwsm}/bin/uwsm start -D Hyprland hyprland-uwsm.desktop'";
        user = "greeter";
      };
    };
  };
